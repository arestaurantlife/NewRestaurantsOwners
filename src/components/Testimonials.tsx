  ...
  return <section id="testimonials" className="py-20 md:py-32 bg-cream-dark">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-lg text-muted-foreground">Join hundreds of restaurant owners who have transformed their businesses with our proven systems.</p>
        </div>

        {/* Testimonials Grid */}
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map((testimonial, index) => <div key={index} className="bg-card rounded-2xl p-6 lg:p-8 shadow-soft border border-border hover:shadow-elevated transition-all duration-300">
              {/* Stars */}
              <div className="flex gap-1 mb-4">
                {Array(testimonial.rating).fill(0).map((_, i) => <svg key={i} className="w-5 h-5 text-gold fill-current" viewBox="0 0 20 20">
